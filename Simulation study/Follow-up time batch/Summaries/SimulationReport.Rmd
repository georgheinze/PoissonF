...
LXPMLE_tab.plot<-cbind(LXPMLE_NA_tab1.plot[,1:4], Beta1_na=LXPMLE_NA_tab1.plot[,5], Beta2_na=LXPMLE_NA_tab2.plot[,5], 
...
p.lxpmle1 <- nested_loop_plot(resdf = LXPMLE_tab.plot[,c("ncov", "n","beta1", "Beta1_na", "LowerCI1_na", "UpperCI1_na")],
                          x = "beta1", steps = "n",
                          grid_rows = "ncov",
                          colors=col,
                          hline_intercept = 0,
...
p.lxpmle2 <- nested_loop_plot(resdf = LXPMLE_tab.plot[,c("ncov", "n","beta1", "Beta2_na", "LowerCI2_na", "UpperCI2_na")],
                          x = "beta1", steps = "n",
                          grid_rows = "ncov",
                          colors=col,
                          hline_intercept = 0,
...
labs <- list(ncov=c('2'="k=2", '5'="k=5", "10"="k=10"))
names(RMSPEN_tab)[5:8] <- c("ML", "FL", "FLAC","DAP")
...
```{r}
leftcov1_tab<-show_tab(methods=c("ML","Firth","LXPMLE","DatAug","Exact","Mid-P"), stat="leftcov1")
rightcov1_tab<-show_tab(methods=c("ML","Firth","LXPMLE","DatAug","Exact","Mid-P"), stat="rightcov1")
colnames(leftcov1_tab) <- c(colnames(leftcov1_tab)[1:4],paste0("left_",colnames(leftcov1_tab)[5:10]))
colnames(rightcov1_tab) <- c(colnames(rightcov1_tab)[1:4],paste0("right_",colnames(rightcov1_tab)[5:10]))
cov1_tab <- cbind(leftcov1_tab, rightcov1_tab[,5:10]) %>% data.frame()

nested_loop_plot(
    cov1_tab[,-c(2,7,13)], x = "beta1", grid_rows = "ncov", steps = "n", 
    colors = col,
    legend_breaks = c("left_ML", "left_Firth", "left_DatAug", "left_Exact", "left_Mid-P"),
    legend_labels = c("ML", "Firth", "DatAug", "Exact", "Mid-P"),
    steps_y_base = 0.81, steps_y_height = 0.01, 
    steps_values_annotate = TRUE, 
    steps_annotation_size = 3,
    design_type = 'partial',
    y_name = "Coverage",
    y_breaks = c(0.825, 0.85, 0.875, 0.925, 0.95, 0.975)
)
```
...
```{r}
leftcov2_tab<-show_tab(methods=c("ML","Firth","LXPMLE","DatAug","Exact","Mid-P"), stat="leftcov2", multi=1000)
rightcov2_tab<-show_tab(methods=c("ML","Firth","LXPMLE","DatAug","Exact","Mid-P"), stat="rightcov2", multi=1000)
