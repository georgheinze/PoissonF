...
* implements ridge regression (normal prior) for Poisson regression with fixed penalty parameter by data augmentation and approximation;
...
* priorinterval ... upper limit of prior interval for exp(beta) (see above), one value for each variable. 
